modelInfo <- list(label = "Boosted Logistic Regression",
                  ...
                  fit = function(x, y, wts, param, lev, last, classProbs, ...) {
                    ## There is another package with a function called `LogitBoostTesting`
                    ## so we call using the namespace
                    TestingTools::LogitBoostTesting(as.matrix(x), y, nIter = param$nIter)
                  },
                  predict = function(modelFit, newdata, submodels = NULL) {
                    ## This model was fit with the maximum value of nIter
                    out <- TestingTools::predict.LogitBoostTesting(modelFit, newdata, type="class")
                    ...
                    {                   
                      ...
                      {
                        out[[j+1]] <- TestingTools::predict.LogitBoostTesting(modelFit,
                                                                  newdata,
                  ...
                  prob = function(modelFit, newdata, submodels = NULL) {
                    out <- TestingTools::predict.LogitBoostTesting(modelFit, newdata, type = "raw")
                    ...
                    if(!is.null(submodels))
                    {
                      ...
                      for(j in seq(along = submodels$nIter))
                      {                           
                        tmpProb <- TestingTools::predict.LogitBoostTesting(modelFit,
                                                               newdata,
                                                               type = "raw",
                                                               nIter = submodels$nIter[j])
